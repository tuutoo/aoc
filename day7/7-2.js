...
totalSize = getSize('/')
freeSize = totalSize - 40000000

let bigFolders = []

let folderArr = dataArr.filter(x => x.prop === 'd' && x.path !== '/')
folderArr.map(d => {
  let size = getSize(d.path)
  if(size >= freeSize) bigFolders.push(size)
})

bigFolders.sort(function(a, b){return a-b});
console.log('bigFolders: ', bigFolders)